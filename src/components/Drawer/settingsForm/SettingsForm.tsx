...
const SettingsForm = ({ onSubmit, onClose }: SettingsFormProps) => {
    ...
    const fetchPayments = async () => {
        ...
        setErrorMessage('');
        try {
            const response = await JC_Services('JAPPCARE', `user-payment-option`, 'GET', null, token);
            ...
            if (response && response.body?.meta?.statusCode === 200) {
                ...
                const errorMsg = response?.body?.meta?.message || 'Failed to fetch payments';
    ...
    return (
        <Box>
            ...
            <Stack spacing={3}>
                ...
                <Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M15 15C15 15.8284 15.6716 16.5 16.5 16.5C17.3284 16.5 18 15.8284 18 15C18 14.1716 17.3284 13.5 16.5 13.5C15.6716 13.5 15 14.1716 15 15Z" stroke="#111111" strokeWidth="1.5" />
                            ...
                            <path d="M17.6258 8C18.0035 6.57673 18.3453 3.98822 17.327 2.70292C16.6816 1.88827 15.7223 1.96654 14.7818 2.04926C9.83791 2.48406 6.34544 3.36731 4.39301 3.96737C3.55348 4.2254 3 5.04522 3 5.96044" stroke="#111111" strokeWidth="1.5" strokeLinejoin="round" />
                        </svg>
                    ...
                    {paymentOptions.length > 0 ? (
                        <Stack spacing={2} sx={{ pl: 4 }}>
                            {paymentOptions.map((option, index) => (
                                <Box key={index} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Typography>{option.paymentMethod}</Typography>
                                    <Typography 
                                        sx={{ 
                                            color: option.approved === 'true' || option.approved === '1' ? 'success.main' : 'error.main',
                                            fontWeight: 'medium'
                                        }}
                                    >
                                        {option.approved === 'true' || option.approved === '1' ? 'Active' : 'Inactive'}
                                    </Typography>
                                </Box>
                            ))}
                        </Stack>
                        ...
                        <Box sx={{ pl: 4, py: 2 }}>
                            <Typography color="text.secondary">
                                No payment methods configured. Please contact support to set up payment options.
                            </Typography>
                        </Box>
                    )}
                </Box>
                ...
                {/* Save Button - Only show if there are payment methods */}
                {paymentOptions.length > 0 && (
                    <Box sx={{ mt: 4 }}>
                        <Button
                            fullWidth
