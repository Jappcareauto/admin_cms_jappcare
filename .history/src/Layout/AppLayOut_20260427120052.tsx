...
import { Box, CssBaseline, GlobalStyles, ThemeProvider, createTheme, useMediaQuery } from '@mui/material';
...
const AppLayout = ({ children }: { children?: React.ReactNode }) => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
            const callbackWindow = window as Window & { requestIdleCallback: (cb: () => void) => number };
            callbackWindow.requestIdleCallback(preloadRoutes);
        } else {
            const timer = setTimeout(preloadRoutes, 1200);
