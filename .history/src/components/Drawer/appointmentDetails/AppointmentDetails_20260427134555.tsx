...
const AppointmentDetails = ({
    ...
    const vehicleLabel = `${vehicle?.make || 'Unknown'} ${vehicle?.model || 'Vehicle'}`;
    const vehicleSubtitle = `${vehicle?.year || 'N/A'}, ${vehicle?.registrationNumber || 'N/A'}`;
    const safeDate = appointment?.date ? parseISO(appointment.date) : null;

    // Placeholder images array - replace with actual vehicle media if available
    const images = vehicle?.media?.items?.length
        ? vehicle.media.items.map((item) => item.sourceUrl)
    ...
    return (
        <Box>
            ...
            <Box
                component="img"
                src={vehicle?.imageUrl || '/api/placeholder/400/320'}
                alt={vehicleLabel}
                sx={{
                    width: '100%',
                    height: 'auto',
            ...
            <Box sx={{
                ...
                <Avatar sx={{
                    ...
                    {(vehicle?.name || 'NA').substring(0, 2).toUpperCase()}
                </Avatar>
                <Typography variant="body1" sx={{ fontWeight: 500 }}>
                    {formatValue(vehicle?.name || 'Unknown customer')}
                </Typography>
                <Chip
            ...
            >
                {formatValue(service?.title || 'Service')}
            </Typography>
            ...
            <Box sx={{ mb: 2 }}>
                <Box sx={{
                    ...
                    <Typography variant="body2">
                        {safeDate ? format(safeDate, 'MMM dd, yyyy') : 'N/A'}
                    </Typography>
                    <Typography variant="body2" sx={{ ml: 1 }}>
                        {safeDate ? format(safeDate, 'hh:mm a') : '--:--'}
                    </Typography>
                    <Typography
                        variant="body2"
                ...
                <Box sx={{
                    ...
                    <Typography variant="body2">
                        {appointment?.locationType === 'HOME' ? 'At Home' : appointment?.locationType || 'N/A'}
                    </Typography>
                    {/* 
