...
const NewServiceProviderForm = ({ onSubmit }: NewServiceProviderFormProps) => {
    ...
    const createServiceCenter = async () => {
        ...
        // Prevent sending default coordinates when user has not chosen a location.
        if (!location || !hasSelectedLocation) {
            setError('Please select a service center location on the map');
            return;
        }
                ...
                <Box sx={{ mb: 2 }}>
                    ...
                    {!googleMapsApiKey && (
                        <Typography variant="caption" color="error.main" sx={{ display: 'block', mt: 1 }}>
                            Google Maps key is missing. Set VITE_GOOGLE_MAPS_API_KEY to enable location selection.
                        </Typography>
                    )}
                </Box>
